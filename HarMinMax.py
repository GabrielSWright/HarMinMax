fromSpec = "" #Chose Native Organism, options listed in line 139, put in the quotes
toSpec = "" #Organism Harmonized to, options listed in line 139, put in the quotes
sequence = "" #paste nucleotide sequence into the string
...
usageFile = fromSpec #input("Input species for usage file (name or number above): ")
...
usageFile2 = toSpec #input("Input species for harmonization: ") CHOSE WHAT TO HARMONIZE TO
...
for i in codonSeq:
    ...
    for j in midList1:
        if i == j[1]:
            value = counter
        counter = counter + 1
    harmonizedCodonSeq.append(midList2[value][1])

hMinMax = calculateMinMax(harmonizedCodonSeq, aaFreqDict2, freqDict2, mapDict, windowSize)

h = 0
flag = 0
while flag < 5:
    totalDist = 0
    for i in range(len(hMinMax)):  
        totalDist += abs(hMinMax[i]-minMaxValues[i])
    print("Iteration: ", h+1)
    print("Initial Distance: ", totalDist)
    start = totalDist
    aboveBelow = [] #0 is below, 1 is above, 2 is equal
    for i in range(len(codonSeq)):
        if minMaxValues[i]>hMinMax[i]:
            aboveBelow.append(0)
        elif minMaxValues[i]<hMinMax[i]:
            aboveBelow.append(1)
        else:
            aboveBelow.append(2)
    ...
    for i in range(len(aboveBelow)):
        if aboveBelow[i] == last:
            ...
            if consecutive >= 10 and last != 2:
                harmonizedCodonSeq2 = list(harmonizedCodonSeq)
                numReplace = int(consecutive/10) 
                replaceStart = i - consecutive
                #print("start: ", replaceStart)
                #print("consecutive: ", consecutive)
                replacePos = []
                for j in range(numReplace):
                    replacePos.append(replaceStart + int(consecutive/(numReplace+1))*(j+1) - 2 + (h%5))
                #print(replacePos)
                for j in replacePos:
                    replaceCodon = harmonizedCodonSeq[j]
                    replaceAA = mapDict[replaceCodon]
                    options = aaMapDict2[replaceAA]
                    optionalCodons = []
                    for k in options:
                        k = k.split()
                        optionalCodons.append(k[0])
                    #print("Codon Choices: ", optionalCodons)
                    replaceFreq = freqDict2[replaceCodon]
                    currentChoice = None
                    for k in optionalCodons:
                        if aboveBelow[i-1] == 0: #window is below, need to raise it
                            if freqDict2[k]>replaceFreq:
                                if currentChoice == None:
                                    currentChoice = k
                                else:
                                    if freqDict2[k]<freqDict2[currentChoice]: #choose lowest synonymous option
                                        currentChoice = k
                        else:
                            if freqDict2[k]<replaceFreq:
                                if currentChoice == None:
                                    ...
                                    if freqDict2[k]>freqDict2[currentChoice]:
                                        currentChoice = k
                    #print(replaceCodon)
                    #print(options)
                    ...
                    #print()
                    if currentChoice != None:
                        harmonizedCodonSeq2[j] = currentChoice
                    
                hMinMax2 = calculateMinMax(harmonizedCodonSeq2, aaFreqDict2, freqDict2, mapDict, windowSize)
                totalDist2 = 0
                for j in range(len(hMinMax2)):
                    totalDist2 += abs(hMinMax2[j]-minMaxValues[j])
                if totalDist2<totalDist:
                    print("Within Iteration Improvement: ", totalDist2)
                    harmonizedCodonSeq = harmonizedCodonSeq2
                    hMinMax = hMinMax2
            ...
            last = aboveBelow[i]
            consecutive = 1
    print("Final from Iteration ", h+1, ": ", totalDist)
    print("")
...
plt.plot(xaxis,minMaxValues, label = "%MinMax", color = '#0000CC')
plt.plot(xaxis, hMinMax, label = "Harmonized %MinMax", color = '#4C9900')
...
plt.ylabel("%MinMax")
plt.savefig('Harmonization_Figure.png', dpi =300)
plt.close()

print("")
print("Length of AA Sequence: " + str(len(minMaxValues)))
print("Number of Harmonized Positions: " + str(len(minMaxValues)-windowSize+1))
print("Average Error Per Position: " + str(totalDist/(len(minMaxValues)-windowSize + 1)))
...
for i in range(len(hMinMax)):
    try:
        if minMaxValues[i]<0:
            numRarePos += 1
            rareError += abs(minMaxValues[i] - hMinMax[i])
    except TypeError:
        pass
print("Error per Rare Position: ", str(rareError/numRarePos))
print("Number Rare Positions: ", str(numRarePos))